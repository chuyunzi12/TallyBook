...
public class TypeBaseAdapter extends BaseAdapter {
    ...
    int type = 0;
    public TypeBaseAdapter(Context context, List<TypeBean> mDatas, int type) {
...
//        判断当前位置是否为选中位置，如果是选中位置，就设置为带颜色的图片，否则为灰色图片
        if (selectPos == position) {
            ...
            if(type == 0){
                iv.setColorFilter(context.getResources().getColor(R.color.theme_color));
            }else {
                iv.setColorFilter(context.getResources().getColor(R.color.theme_blue));
            }
        }else{
            iv.setImageResource(typeBean.getImageId());
            iv.setColorFilter(context.getResources().getColor(R.color.black));
        }
        return convertView;
    }
}
